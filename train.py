...
def split_sequences(sequences: list[list[int]], train_split: float) -> tuple[list[list[int]], list[list[int]]]:
...
def make_batch(sequences: list[list[int]], batch_size: int, seq_len: int, device: torch.device) -> tuple[torch.Tensor, torch.Tensor]:
    batch = random.sample(sequences, k=min(batch_size, len(sequences)))
    inputs = [item[:seq_len] for item in batch]
    targets = [item[1:seq_len + 1] for item in batch]
    return (
...
def evaluate(
    ...
    model.eval()
    inputs, targets = make_batch(sequences, batch_size=batch_size, seq_len=seq_len, device=device)
    logits, aux_list = model(inputs)
    loss = criterion(logits, targets, aux_list)
...
def main() -> None:
    ...
    for step in range(args.steps):
        ...
        inputs, targets = make_batch(train_sequences, batch_size=args.batch_size, seq_len=args.seq_len, device=device)
        ...
        if step % args.log_every == 0 or step == args.steps - 1:
            ...
            print(
                "step={step} loss={loss:.4f} val_loss={val_loss:.4f} lr={lr:.6f} tau_z={tau_z:.3f} "
                "lambda_p={lambda_p:.3f} grad_norm={grad_norm:.3f} mean_active={mean_active:.3f} "
                "expert_load_range=[{min_load:.3f}, {max_load:.3f}]".format(
                    ...
                    mean_active=train_metrics["mean_active_experts"],
